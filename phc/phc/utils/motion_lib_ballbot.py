...
class MotionLibBallbot(MotionLibBase):

    def __init__(self, motion_file, device, fix_height=FixHeightMode.full_fix, masterfoot_conifg=None, min_length=-1, im_eval=False, multi_thread=True, extend_hand = True, extend_head = False, mjcf_file="resources/robots/ballbot/ballbot.xml", sim_timestep = 1/50):
        ...
        self.mesh_parsers = Ballbot_Batch(extend_hand=extend_hand, extend_head=extend_head, mjcf_file=mjcf_file)
        return
    ...
    def fix_trans_height(pose_aa, trans, curr_gender_betas, mesh_parsers, fix_height_mode):
        ...
        with torch.no_grad():
            raise NotImplementedError("Fix height is not implemented for Ballbot")
